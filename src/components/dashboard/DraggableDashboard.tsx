...
function SortableWidget({ id, widget, data }: { id: string, widget: Widget, data: DashboardData }) {
    const {
        attributes,
        listeners,
        setNodeRef,
    ...
    const renderWidget = () => {
        switch (widget.type) {
            case 'tasks':
                return <TasksWidget tasks={data.tasks} isLoading={data.loading} onTaskClick={data.onTaskClick} />
            case 'recents':
                return <RecentsWidget items={data.recents} isLoading={data.loading} onItemClick={data.onRecentClick} />
            case 'calendar':
                return <CalendarWidget tasks={data.calendar} isLoading={data.loading} />
            default:
                return null
        }
    }
    ...
    return (
        <div
            ...
            className={cn(
                ...
                isDragging ? "opacity-50 z-50 overflow-hidden ring-2 ring-primary" : ""
            )}
        >
...
export function DraggableDashboard({ data }: { data: DashboardData }) {
    ...
    const handleDragOver = (event: DragOverEvent) => {
        ...
        if (activeColId !== overColId) {
            setColumns((prev) => {
                const activeColIndex = prev.findIndex(c => c.id === activeColId)
                ...
                const activeWidgetIndex = activeWidgets.findIndex(w => w.id === activeId)
                ...
                if (isOverColumn) {
                    ...
                    const overWidgetIndex = overWidgets.findIndex(w => w.id === overId)
                    ...
                    const newIndex = overWidgetIndex >= 0 ? overWidgetIndex + modifier : overWidgets.length + 1;

                    overWidgets.splice(newIndex, 0, activeWidget)
                }

                const newCols = [...prev]
                newCols[activeColIndex] = { ...newCols[activeColIndex], widgets: activeWidgets }
                newCols[overColIndex] = { ...newCols[overColIndex], widgets: overWidgets }
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        const activeCol = columns.find(c => c.widgets.some(w => w.id === activeId))
        const overCol = columns.find(c => c.widgets.some(w => w.id === overId)) || columns.find(c => c.id === overId)

        if (activeCol && overCol && activeCol.id === overCol.id) {
            const colIndex = columns.findIndex(c => c.id === activeCol.id)
            const oldIndex = activeCol.widgets.findIndex(w => w.id === activeId)
            const newIndex = activeCol.widgets.findIndex(w => w.id === overId)

            if (oldIndex !== newIndex) {
                setColumns(prev => {
                    const next = [...prev]
                    next[colIndex].widgets = arrayMove(next[colIndex].widgets, oldIndex, newIndex)
                    return next
                })
            }
        }
...
function Resizer({ currentWidth, onUpdateWidth }: { currentWidth: number, onUpdateWidth: (w: number) => void }) {
    const handleMouseDown = (e: React.MouseEvent) => {
        e.preventDefault()
        const startX = e.clientX
        const startWidth = currentWidth
        const parentWidth = (e.currentTarget.parentElement?.parentElement?.offsetWidth || 1000)
