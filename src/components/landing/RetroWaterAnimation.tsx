...
function RetroWaterAnimationComponent({
    className = '',
    intensity = 'medium',
}: RetroWaterAnimationProps) {
    ...
    useEffect(() => {
        ...
        const pixelSize =
            intensity === 'subtle' ? 12 : intensity === 'medium' ? 10 : 8;
        ...
        const draw = () => {
            ...
            for (let y = 0; y < rows; y++) {
                for (let x = 0; x < cols; x++) {
                    // Create wave pattern
                    const wave1 =
                        Math.sin(x * 0.1 + time * 0.02 + y * 0.05) * 0.5 + 0.5;
                    const wave2 =
                        Math.sin(x * 0.05 - time * 0.015 + y * 0.08) * 0.5 +
                        0.5;
                    ...
                    const colorIndex =
                        Math.floor(combinedWave * colors.length) %
                        colors.length;
                    const color = colors[colorIndex];

                    // Add shimmer effect
                    const shimmer =
                        Math.sin((x + y) * 0.3 + time * 0.05) * 0.1 + 0.9;
