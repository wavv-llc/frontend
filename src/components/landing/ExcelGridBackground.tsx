...
function ExcelGridBackgroundComponent({
    ...
    const getColumnOpacity = (index: number) => {
        ...
        return startOpacity - (index / (totalColumns - 1)) * (startOpacity - endOpacity)
    }

    const getRowOpacity = (index: number) => {
        const startOpacity = 0.5
        const endOpacity = 0.1
        return startOpacity - (index / (totalRows - 1)) * (startOpacity - endOpacity)
    }
                    ...
                    {columns.map((letter, i) => {
                        ...
                        return (
                            <motion.div
                                ...
                                initial={animated ? { opacity: 0 } : { opacity: targetOpacity }}
