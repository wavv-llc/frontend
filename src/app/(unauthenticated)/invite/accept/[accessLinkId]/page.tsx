            ...
            <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                ...
                >
                    <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center">
                        <span className="text-primary-foreground font-serif italic text-lg pr-0.5">
                            w
                        </span>
                    </div>
                    wavv
...
export default function InviteAcceptPage() {
    ...
    useEffect(() => {
        const validateAccessLink = async () => {
            try {
                const response =
                    await accessLinkApi.getAccessLink(accessLinkId);
    ...
    useEffect(() => {
        ...
        if (userEmail !== inviteEmail) {
            toast.error(
                `You must sign in with ${accessLink.email} to accept this invite.`,
            );
            return;
        }
        ...
        const acceptInvite = async () => {
            ...
            try {
                ...
                console.error(
                    '❌ POST /api/v1/access-links/accept - Error:',
                    err,
                );
                toast.error(
                    err instanceof Error
                            ...
                            >
                                ...
                                <div className="relative bg-card/50 backdrop-blur-xl border border-border rounded-2xl p-10 md:p-12 text-center shadow-xl">
                                    ...
                                    <p className="text-muted-foreground mb-10">
                                        Please sign out and sign in with the
                                        correct account.
                                    </p>
                                    <Link href="/sign-in">
                                        <Button className="rounded-full h-11 px-8 shadow-lg">
                        ...
                        <div className="bg-card/50 backdrop-blur-xl border border-border rounded-2xl p-8 md:p-10 shadow-lg">
                            ...
                            <div className="mt-6 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                                <p className="text-amber-600 dark:text-amber-400 text-sm leading-relaxed">
                                    <strong>Important:</strong> You must sign up
                                    using a Microsoft account with the email
                                    address <strong>{accessLink?.email}</strong>
                                    .
                                </p>
                            </div>
                        </div>
                                ...
                                <SignUp
                                    ...
                                    appearance={{
                                        elements: {
                                            ...
                                            cardBox:
                                                '!bg-transparent w-full flex justify-center',
                                            header: 'hidden',
                                            headerTitle: 'hidden',
                                            headerSubtitle: 'hidden',
                                            main: '!bg-transparent w-full flex justify-center',
                                            main__body:
                                                '!bg-transparent w-full flex flex-col items-center !mt-0',
                                            formButtonPrimary:
                                                '!bg-primary hover:!bg-primary/90 text-primary-foreground shadow-lg hover:shadow-xl transition-all !border-none w-full !rounded-full !h-11',
                                            socialButtonsBlockButton:
                                                'border-border hover:bg-muted/30 transition-colors !bg-background/50 w-full !mt-0 !rounded-full !h-11',
                                            ...
                                            socialButtonsBlockButtonText:
                                                'text-foreground',
                                            formFieldInput:
                                                'border-border focus:border-primary focus:ring-primary !bg-background/50 w-full !rounded-xl',
                                            formFieldLabel:
                                                'text-foreground font-medium',
                                            formField: 'w-full',
                                            form: 'w-full flex flex-col items-center',
                                            identityPreview:
                                                '!bg-transparent border-border w-full',
                                            identityPreviewText:
                                                'text-foreground',
